...
S = fresh()
check("display defaults on", S.getShowLogistics())
check("display can be disabled", S.setShowLogistics(False))
S.create("Visible chain", recs(("10", "AAA-001")))
S.create("Other chain", recs(("20", "BBB-002")))
S.select(1)
S.rename(1, "Renamed")
S.addStations(1, recs(("30", "CCC-003")))
S.removeStation(1, "30")
S.setWarningIgnored("AAA-001", "ore", True)
S.delete(2)
S = reload_file()
check("false survives reload and chain mutations", not S.getShowLogistics())
check("storage version advanced", lua.globals().__SCV_GROUPS.version == 6)
check("stored chains and warnings retained", S.count() == 1 and S.isWarningIgnored("AAA-001", "ore"))
check("invalid setter rejected", not S.setShowLogistics("false") and not S.getShowLogistics())
S.setShowLogistics(True)
check("true survives reload", reload_file().getShowLogistics())
for value in ('nil', '"false"', '0', '{}'):
    lua.execute('__SCV_GROUPS = {version=5, names={"Legacy"}, members={"10|AAA-001"}, showLogistics=' + value + '}')
    S = reload_file()
    check("legacy or invalid preference defaults on: " + value, S.getShowLogistics() and S.count() == 1)
